...
impl ClientShellState {
    ...
    fn prepare_committed_text(&mut self, text: &str, outcome: &mut ClientShellInput) -> bool {
        if self.insert_sidebar_text(text) || self.insert_copy_search_text(text) {
    ...
    pub(super) fn modal_paste_target_active(&self) -> bool {
        ...
        if self.overlay.is_none() && self.sidebar_prompt_active() {
            return true;
        }
        if self
            .copy_mode
    ...
    pub(super) fn handle_modal_paste_shortcut_with(
        ...
        if let Some(text) = read_clipboard_text() {
            let inserted = self.insert_sidebar_text(&text)
    ...
    fn route_key_press(
        ...
        if self.route_sidebar_prompt(key, outcome) {
            return None;
        }
    ...
    fn route_navigate_key(
        ...
        if self.route_sidebar_navigation(key, outcome) {
            return;
        }
        if key.code == KeyCode::Esc
    ...
    fn record_navigate_binding(
        ...
        if let KeybindMatch::Action(
            action @ (KeybindAction::JumpSidebarItem(_) | KeybindAction::JumpSidebarItemPrompt),
        ) = binding
        {
            self.handle_sidebar_action(action, outcome);
            return;
        }
        if matches!(
            binding,
        ...
        ) && !self.prepare_sidebar_workspace_action(outcome)
        {
            return;
        }
