...
// The fork uses the pane tree when expanded; workspace-only navigation remains
// active in the compact sidebar and mobile layout.
fn state_with_remote() -> (ClientShellState, ClientEndpointId) {
    let (mut state, remote) = super::state_with_remote();
...
fn enter_navigation(state: &mut ClientShellState) {
    ...
    // Opening the fork's picker expands the tree. Exercise the retained compact
    // navigation path explicitly; expanded-tree navigation has its own suite.
...
fn navigation_highlights_only_the_preview_and_activates_on_enter() {
    for (compact, cols) in [(true, 100), (false, 44)] {
