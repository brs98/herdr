...
fn grouped_worktrees_render_parent_branch_and_indented_child() {
    ...
    assert!(state
        ...
        .workspaces
        .iter()
        .any(|hit| hit.workspace_id == "ws_2" && hit.indented));
    ...
    let parent_row = &collapsed.cells[usize::from(parent.y) * usize::from(collapsed.width)..]
        [..usize::from(parent.width)];
    assert!(parent_row
        .iter()
        .any(|cell| cell.fg == crate::protocol::color_to_u32(state.config.palette.red)));
...
#[test]
fn tree_sidebar_preserves_tokens_symbols_and_pane_order() {
    let mut projected = snapshot();
    ...
    let pane_hits = state
        ...
        .filter_map(|(rect, target)| {
            if let ClientNavigatorTarget::Pane { pane_id, .. } = target {
                Some((*rect, pane_id.as_str()))
            } else {
                None
            }
        })
        .collect::<Vec<_>>();
    assert_eq!(
        pane_hits.iter().map(|(_, id)| *id).collect::<Vec<_>>(),
        vec!["pane_1", "pane_2"]
    );
    let first = pane_hits[1].0;
    let click = state.handle_raw_events(vec![RawInputEvent::Mouse(crossterm::event::MouseEvent {
        kind: MouseEventKind::Down(MouseButton::Left),
        column: first.x,
        row: first.y,
    ...
    state.compose(106, 10).expect("short tree sidebar frame");
    assert!(state
        .hits
        .sidebar_tree_rows
        .iter()
        .any(|(_, target)| matches!(target,
        ClientNavigatorTarget::Pane { pane_id, .. } if pane_id == "pane_2")));
...
fn muted_agent_sidebar_rows_do_not_stack_terminal_faint() {
    ...
    let row = hits.agents.first().expect("agent row hit").0;
...
#[test]
fn agent_view_focus_indices_remain_independent_of_tree_order() {
    let mut projected = snapshot();
    let mut second_pane = projected.panes[0].clone();
    second_pane.pane_id = "pane_2".into();
    second_pane.focused = false;
...
#[test]
fn agent_sort_preference_persists_without_a_tree_sort_toggle() {
    ...
    let mut persisted = ClientShellInput::default();
    state.persist_chrome_preferences(&mut persisted);
...
fn navigate_mode_selects_workspace_locally_then_focuses_by_stable_id() {
    ...
    assert!(matches!(state.sidebar_tree.selected.as_ref(),
        Some(ClientNavigatorTarget::Workspace { workspace_id, .. }) if workspace_id == "ws_2"));
