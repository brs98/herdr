...
fn compose_agent_panel(state: &mut ClientShellState, cols: u16, rows: u16) -> Option<FrameData> {
    state.last_composed_size = Some((cols, rows));
    let mut buffer = Buffer::empty(Rect::new(0, 0, cols, rows));
    let (_, area) = crate::ui::expanded_sidebar_sections(
        state.layout(cols, rows).sidebar,
        state.sidebar_section_split,
    );
    state.hits = ShellHitMap::default();
    super::super::endpoint_agents::render_expanded(
        ...
        &state.config,
        &mut state.agent_scroll,
        &mut state.hits,
    );
    Some(FrameData::from_ratatui_buffer(&buffer, None))
}
...
fn state_with_scrollable_agents() -> (ClientShellState, ClientEndpointId) {
    ...
    compose_agent_panel(&mut state, 100, 28).unwrap();
...
fn sidebar_renders_local_and_saved_ssh_endpoints_with_status() {
    ...
    state.config.sidebar_max_width = 60;
    state.sidebar_width = 60;
    state.sidebar_width_manual = true;
    let frame = state.compose(160, 28).expect("combined endpoint frame");
    let text = frame
        .cells
...
fn saved_machine_preserves_endpoint_scoped_worktree_collapses() {
    ...
    let local = state
        ...
        .unwrap();
    assert_eq!(
        sidebar::displayed_workspace_status(local, &local.workspaces[0], &state.collapsed_groups),
        AgentStatus::Blocked
    );
...
fn expanded_machine_sidebar_reveals_newly_focused_workspace() {
    ...
    state.compose(106, 12).expect("full machines sidebar");
    ...
    state.compose(106, 12).expect("new workspace revealed");
    assert!(state
        .hits
        .workspaces
        .iter()
    ...
    state.compose(106, 12).expect("manual scroll");
    assert_eq!(state.workspace_scroll, 0);
    assert!(!state
        .hits
        .workspaces
...
fn expanded_machine_sidebar_applies_space_row_gap_within_each_machine() {
    ...
    state.compose(100, 8).expect("scrolled endpoint frame");
    let metrics = state
        .hits
...
#[test]
fn tree_highlights_focused_pane_and_selected_collapsed_machine() {
    ...
    let collapse = state.handle_input_bytes(b" ");
...
fn aggregate_agents_use_configured_rows_machine_token_and_status_colors() {
    ...
    state.config.sidebar_max_width = 70;
    state.sidebar_width = 70;
    state.sidebar_width_manual = true;
    let profile = remote_profile();
    let endpoint_id = ClientEndpointId::Ssh(profile.id.clone());
    state.set_endpoint_catalog(&[profile]);
...
fn current_workspace_agent_view_excludes_same_workspace_id_on_other_machine() {
    ...
    let frame = compose_agent_panel(&mut state, 100, 28).expect("combined endpoint frame");
    let text = frame
        .cells
        .chunks(frame.width as usize)
        .map(|row| {
    ...
    remote_surface.boot_id = "remote-boot".into();
    state.set_pane_surface(remote_surface);
    let frame = compose_agent_panel(&mut state, 100, 28).expect("remote endpoint frame");
    let text = frame
        .cells
        .chunks(frame.width as usize)
        .map(|row| {
...
fn legacy_custom_views_keep_v1_per_endpoint_projection() {
    ...
    let frame = compose_agent_panel(&mut state, 100, 28).expect("legacy combined frame");
    let text = frame
        .cells
        .chunks(frame.width as usize)
        .map(|row| {
...
fn selected_custom_sort_orders_rendering_and_indexed_navigation() {
    ...
    let frame = compose_agent_panel(&mut state, 100, 28).expect("custom sorted frame");
    let text = frame
        .cells
        .chunks(frame.width as usize)
        .map(|row| {
...
#[test]
fn aggregate_priority_navigation_uses_recency_without_reordering_the_tree() {
    use crate::api::schema::AgentStatus;
    use crate::config::AgentSidebarToken;
        ...
        for status in [
            ...
            for collapsed in [true, false] {
                ...
                let machine = state
                    ...
                    .find(|hit| hit.endpoint_id == remote_id)
                    .expect("remote machine");
                let column = machine.collapse_toggle.x;
                let machine = machine.rect;
