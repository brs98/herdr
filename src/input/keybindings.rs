...
pub(crate) fn resolve_non_indexed_action(
    ...
    for (bindings, action) in [
        ...
        (
            ...
            KeybindAction::JumpSidebarItemPrompt,
        ),
...
pub(crate) fn resolve_indexed_action(
    ...
    for exact_modifiers in [true, false] {
        ...
        for binding in &keybinds.jump_sidebar_item {
            if trigger_matches(binding) {
                if let Some(index) = binding.matched_index(key) {
                    return Some(KeybindAction::JumpSidebarItem(index));
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn sidebar_jumps_resolve_configured_numbers_and_prompt() {
...
"#,
        ...
        .expect("valid sidebar bindings");
        let keybinds = config.keybinds();
        for (key, expected) in [
            ('1', KeybindAction::JumpSidebarItem(0)),
            ('9', KeybindAction::JumpSidebarItem(8)),
            ('0', KeybindAction::JumpSidebarItem(9)),
            ('j', KeybindAction::JumpSidebarItemPrompt),
        ] {
            assert!(matches!(
                resolve_direct_binding(&keybinds, &TerminalKey::new(KeyCode::Char(key), KeyModifiers::ALT)),
                Some(KeybindMatch::Action(action)) if action == expected
            ));
        }
    }
}
