...
`agent.view.set` installs one transient declarative projection for Agent lists.
The projection is reevaluated whenever agent facts or current UI context change.
It controls the compact collapsed rail, mobile Agents list, their mouse targets,
indexed focus, and next/previous Agent navigation. The expanded session tree
always keeps its full workspace, tab, and pane hierarchy; this projection does
not filter or reorder that tree. It also does not change `agent.list`,
notifications, detection, or global attention counts.
...
Workspace get/list responses expose the resulting `tokens` map, and workspace-node layouts can render values such as `$jj_status`. Changes and TTL expiry emit `workspace.metadata_updated` with the latest workspace snapshot. This metadata event is available to API subscribers but does not invoke plugin event hooks.
