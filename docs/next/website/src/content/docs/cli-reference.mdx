...
`--token` patches one named display value; `--clear-token` removes one. Unmentioned tokens remain unchanged. Pane tokens are available to Agent pane-node layouts as `$name`; workspace tokens are available to workspace-node layouts. TTL applies independently to the token keys updated by that call.
