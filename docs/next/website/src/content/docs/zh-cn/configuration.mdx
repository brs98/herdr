...
按键字符串支持普通按键、`ctrl+a`、`shift+n`、`alt+1`、`cmd+k` 等修饰组合键，以及 `enter`、`tab`、`esc`、`left`、`right`、`up`、`down` 等特殊键。也支持 `minus`、`comma`、`ampersand`、`plus`、`backtick` 等命名标点。直接绑定 `n` 这样的普通可打印键并不安全，因为它会拦截输入；除非你有意设置直接绑定，否则请使用 `prefix+n`。`navigate_workspace_*` 和 `navigate_pane_*` 字段仅在 Navigate 模式中生效，可以使用 `j` 或 `k` 等普通按键；它们不得使用 `prefix+`、`esc`、`enter`、`tab`、`shift+tab`、`left`、`right`，也不得使用无修饰键的 `1` 到 `9`。在桌面侧边栏树中，上下绑定与 `navigate_pane_up/down` 会在可选择的工作区行、标签页行和窗格行之间移动；`navigate_pane_left/right` 与左右方向键用于折叠、展开或在父子节点之间移动。这些快捷键独立于 `focus_pane_down = "prefix+j"` 等通用动作绑定；Navigate 模式打开时，本地导航快捷键优先。Alt、Cmd/Super 以及带修饰键的标点取决于你的终端和 tmux 设置。
...
侧边栏树只为当前可见的窗格行编号。工作区行和拆分标签页的分组标签不显示编号，但两者仍可通过搜索或方向键导航。标签页行可以在不占用跳转编号的情况下选择、折叠和展开。`jump_sidebar_item` 可直接打开编号 1 至 10（`0` 表示编号 10）。若要跳转到任意编号，请使用 `jump_sidebar_item_prompt`，输入完整编号后按 Enter。折叠树节点或筛选结果变化时，可见窗格会从 1 重新编号。只有一个窗格的标签页会合并为该窗格的一行；拆分后的标签页会在无编号的标签页标签下显示各个带编号的窗格。
...
侧边栏是 Herdr 的主仪表盘。展开的桌面视图是一棵会话树：工作区包含标签页，标签页包含 shell 或 Agent 窗格。连接多台机器时，机器行会将各自的会话分组。只有一个窗格的标签页会合并为一行；拆分后的标签页会在无编号且可选择的标签页行下显示各个带编号的窗格。点击或选择工作区、标签页或窗格即可聚焦该项。按 `prefix+w` 可用键盘聚焦该树，再使用 `j`/`k`、`h`/`l`、Enter 和 `/` 移动、折叠/展开、激活和就地筛选。在 Navigate 模式中，选中行会使用当前主题的强调色；在 Navigate 模式之外，当前聚焦的窗格会保持相同的强调色背景。在[配置参考](/docs/config-reference/)中搜索 `ui.`，可查看尺寸、折叠模式、鼠标行为、窗格边框和其他显示设置。`ui.agent_panel_sort` 仍控制 Agent 循环切换顺序和折叠后的紧凑栏，但不会改变层级树的顺序。
...
现有的 Space 和 Agent 布局设置用于设置树中工作区节点和已检测 Agent 窗格节点的样式。每个树节点占用一个终端行。Herdr 会解析 `rows` 中的每个内层数组，再按顺序将其中的非空值展平到该节点的一行中。以下是完整的默认布局：
...
token 会按配置顺序渲染。Herdr 通常使用 ` · ` 分隔相邻值，并在 `state_icon` 后使用一个空格。缺失值及其分隔符会消失；没有任何值的内层数组不会向节点行添加内容。每个布局最多可包含 16 个内层数组，每个数组最多可包含 16 个 token。 已有的自定义行不会改变；如果希望在自定义的多机器布局中显示机器标识，请显式添加 `machine`。
...
树节点会刻意以每个终端行一个节点的方式紧密排列，因此 `row_gap` 仅为兼容性保留，不会在展开的树中添加间距。

在 `rows_by_agent` 下为已知智能体覆盖完整的 Agent 窗格节点布局：
...
覆盖项会替换 `rows`，而不是扩展它。其内层数组也会以相同方式展平到窗格节点的一行中。覆盖项的键是区分大小写的规范智能体 ID，例如 `claude`、`codex` 和 `pi`。不接受 `claude-code` 等检测别名。没有覆盖项的智能体（包括自定义报告的智能体）会使用 `rows`。
...
侧边栏节点设置仅影响展开的桌面树。shell 窗格没有 Agent token 上下文，因此使用解析后的窗格标题或标签。折叠视图和移动视图仍使用紧凑布局。
...
Herdr 默认在兼容的外层终端中渲染窗格图像。图像放置区域与弹窗、菜单或通知重叠时，Herdr 会暂时隐藏整个图像放置区域，而不是只裁剪重叠部分。未被遮挡的图像保持可见，被隐藏的图像放置区域在遮挡消失后恢复显示。图像不会随对话框背景一起变暗。

要禁用图形渲染和窗格图形 API，请设置：
...
更改此设置后，需要重启受影响的 Herdr 服务器或重新连接客户端。在远程会话中，服务器端设置控制窗格图形解析和 API 可用性，本地客户端设置控制向外层终端输出图形。
