...
侧边栏是 Herdr 的主仪表盘。展开的桌面视图是一棵会话树：工作区包含标签页，标签页包含 shell 或 Agent 窗格。即使工作区只有一个标签页，也会明确显示该标签页。点击任意节点即可聚焦对应的工作区、标签页或窗格。按 `prefix+w` 可用键盘聚焦该树，再使用 `j`/`k`、`h`/`l`、Enter 和 `/` 移动、折叠/展开、激活和就地筛选。在[配置参考](/docs/config-reference/)中搜索 `ui.`，可查看尺寸、折叠模式、鼠标行为、窗格边框和其他显示设置。`ui.agent_panel_sort` 仍控制 Agent 循环切换顺序和折叠后的紧凑栏，但不会改变层级树的顺序。
