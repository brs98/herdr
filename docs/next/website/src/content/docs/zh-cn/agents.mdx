...
`state` 控制等待、通知和汇总。`summary` 令牌只影响展示,可在 Agent 窗格节点布局中写成 `$summary`。

Agent 窗格节点布局也可以选择使用 `terminal_title` 或 `terminal_title_stripped`;两者都不在默认布局中。前者显示经过安全规范化的最新 OSC 0/2 终端标题,后者会移除开头一个已识别的活动或旋转指示符字形及其后的空白。这些值由 Herdr 服务器所有,冷重启后不会保留,并且独立于元数据标题和语义智能体状态。因此,如果移除后的文本不变,旋转动画可以改变原始标题而不触发窗格更新。
