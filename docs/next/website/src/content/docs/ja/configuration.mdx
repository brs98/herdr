...
キー文字列には、通常のキー、`ctrl+a`、`shift+n`、`alt+1`、`cmd+k` のような修飾キーの組み合わせ、`enter`、`tab`、`esc`、`left`、`right`、`up`、`down` のような特殊キーが使えます。`minus`、`comma`、`ampersand`、`plus`、`backtick` のような名前付き記号も使えます。`n` のような通常の印字可能キーの直接バインドは入力を妨げるため危険です。意図的に直接バインドするのでなければ `prefix+n` を使ってください。`navigate_workspace_*` と `navigate_pane_*` のフィールドは Navigate モード専用で、`j` や `k` のような通常のキーを使えます。これらには `prefix+`、`esc`、`enter`、`tab`、`shift+tab`、`left`、`right`、修飾なしの `1` から `9` は使えません。デスクトップのサイドバーツリーでは、上下のバインドと `navigate_pane_up/down` が、選択可能なワークスペース行、タブ行、ペイン行の間を移動します。`navigate_pane_left/right` と左右の矢印キーは、ノードの折りたたみ、展開、親子間の移動を行います。これらは `focus_pane_down = "prefix+j"` のような一般アクションのバインドから独立しており、Navigate モード中はローカルショートカットが優先されます。Alt、Cmd/Super、修飾キー付き記号は、ターミナルと tmux の設定に依存します。
...
サイドバーツリーでは、表示中のペイン行だけに番号が付きます。ワークスペース行と分割タブのグループラベルには番号が付きませんが、どちらも検索や矢印キーで引き続き移動できます。タブ行はジャンプ番号を消費せずに選択、折りたたみ、展開できます。`jump_sidebar_item` は番号付きの 1〜10 番を直接開きます（`0` は 10 番）。任意の番号へ移動するには、`jump_sidebar_item_prompt` を使い、完全な番号を入力して Enter を押します。ツリーの折りたたみやフィルターに応じて表示項目が変わると、表示中のペインは 1 から振り直されます。ペインが 1 つだけのタブは、そのペインの 1 行にまとめられます。分割されたタブでは、番号なしのタブラベルの下に番号付きの各ペインが表示されます。
...
サイドバーは Herdr のメインダッシュボードです。展開されたデスクトップ表示は 1 つのセッションツリーで、ワークスペースの下にタブ、タブの下にシェルまたは Agent のペインが並びます。複数のマシンに接続している場合、各マシンの行の下にセッションがまとめられます。ペインが 1 つだけのタブは 1 つのペイン行にまとめられ、分割されたタブでは番号なしの選択可能なタブ行の下に、番号付きの各ペインが表示されます。ワークスペース、タブ、ペインをクリックまたは選択すると、その項目にフォーカスします。`prefix+w` でツリーにフォーカスし、`j`/`k`、`h`/`l`、Enter、`/` で移動、折りたたみ/展開、選択、絞り込みを行えます。Navigate モードでは選択中の行に現在のテーマのアクセント色が使われ、Navigate モード以外では現在フォーカス中のペインに同じアクセント背景が維持されます。サイズ、折りたたみモード、マウスの挙動、ペインの境界線、その他の表示設定については、[設定リファレンス](/docs/config-reference/)で `ui.` を検索してください。`ui.agent_panel_sort` は引き続き Agent の切り替え順と折りたたまれたコンパクトレールを制御しますが、階層ツリーの順序は変更しません。
...
既存の Space と Agent のレイアウト設定は、ツリー内のワークスペースノードと、検出された Agent のペインノードを装飾します。各ツリーノードはターミナルの 1 行を使います。Herdr は `rows` の各内側の配列を解決し、値のある内容を順番にそのノードの 1 行へ連結します。完全なデフォルトレイアウトは次のとおりです:
...
トークンは設定された順序で描画されます。Herdr は通常、隣接する値を ` · ` で区切り、`state_icon` の後には空白を 1 つ入れます。値がない場合、その値と区切りは表示されません。値が 1 つもない内側の配列はノード行に何も追加しません。各レイアウトは最大 16 個の内側の配列、各配列は最大 16 トークンです。 既存のカスタム行は変更されません。カスタムの複数マシンレイアウトでマシンを識別したい場合は、`machine` を明示的に追加してください。
...
ツリーノードは意図的にターミナルの 1 行ずつ詰めて表示されるため、`row_gap` は互換性のために残されていますが、展開されたツリーに間隔を追加しません。

既知のエージェントについて Agent ペインノードの完全なレイアウトを上書きするには、`rows_by_agent` を使います:
...
上書きは `rows` を置き換えるもので、拡張するものではありません。内側の配列は通常のレイアウトと同じようにペインノードの 1 行へ連結されます。上書きのキーには、`claude`、`codex`、`pi` のような大文字と小文字を区別する正規エージェント ID を使います。`claude-code` のような検出エイリアスは使えません。カスタム報告されたエージェントを含め、上書きがないエージェントには `rows` が使われます。
...
サイドバーのノード設定は、展開されたデスクトップツリーにだけ適用されます。シェルペインには Agent トークンのコンテキストがないため、解決済みのペインタイトルまたはラベルが使われます。折りたたみ表示とモバイル表示はコンパクトなレイアウトを保ちます。
...
Herdr は、対応する外側のターミナルでペイン画像をデフォルトで描画します。グラフィックス描画とペイングラフィックス API を無効にするには、次のように設定します:
