...
トークンは設定された順序で描画されます。Herdr は通常、隣接する値を ` · ` で区切り、`state_icon` の後には空白を 1 つ入れます。値がない場合、その値と区切りは表示されません。値が 1 つもない内側の配列はノード行に何も追加しません。各レイアウトは最大 16 個の内側の配列、各配列は最大 16 トークンです。
