...
サイドバーは Herdr のメインダッシュボードです。展開されたデスクトップ表示は 1 つのセッションツリーで、ワークスペースの下にタブ、タブの下にシェルまたは Agent のペインが並びます。タブが 1 つしかないワークスペースでも、そのタブは常に表示されます。任意のノードをクリックすると、対応するワークスペース、タブ、またはペインにフォーカスします。`prefix+w` でツリーにフォーカスし、`j`/`k`、`h`/`l`、Enter、`/` で移動、折りたたみ/展開、選択、絞り込みを行えます。サイズ、折りたたみモード、マウスの挙動、ペインの境界線、その他の表示設定については、[設定リファレンス](/docs/config-reference/)で `ui.` を検索してください。`ui.agent_panel_sort` は引き続き Agent の切り替え順と折りたたまれたコンパクトレールを制御しますが、階層ツリーの順序は変更しません。
