...
`--token` は名前付き表示値を設定し、`--clear-token` は削除します。指定しなかったトークンは変わりません。ペイントークンは Agent ペインノードのレイアウトで `$name` として、ワークスペーストークンはワークスペースノードのレイアウトで使えます。TTL は、その呼び出しで更新した各トークンキーに個別に適用されます。
