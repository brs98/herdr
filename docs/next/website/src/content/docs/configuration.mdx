...
The sidebar is the main Herdr dashboard. Its expanded desktop view is one session tree: workspaces contain tabs, and tabs contain shell or Agent panes. Every tab remains visible even when a workspace has only one. Clicking any node focuses that workspace, tab, or pane. Press `prefix+w` to focus the tree from the keyboard, then use `j`/`k`, `h`/`l`, Enter, and `/` for movement, collapse/expand, activation, and inline filtering. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, mouse behavior, pane borders, and other presentation settings. `ui.agent_panel_sort` still controls Agent cycling and the compact collapsed rail; it does not reorder the hierarchical tree.
