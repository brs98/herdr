...
The sidebar tree numbers every visible pane. Workspace rows and split-tab group labels stay
unnumbered, but both remain reachable with search or arrow navigation. Tab rows can be activated,
collapsed, and expanded without consuming a jump number. `jump_sidebar_item` opens
numbered targets 1 through 10 directly (`0` means target 10). To jump to any numbered
target, use `jump_sidebar_item_prompt`, type the full number, and press Enter. Collapse
and filtering renumber the visible panes from 1. A tab with one pane is condensed into that
pane's single numbered row; split tabs keep an unnumbered tab label above their numbered panes.
...
The sidebar is the main Herdr dashboard. Its expanded desktop view is one session tree: workspaces contain tabs, and tabs contain shell or Agent panes. When several machines are connected, machine rows group their sessions and use the theme accent color. Tab headings appear only when a space has multiple tabs and the tab does not contain exactly one pane. A single-pane tab shows the pane name directly beneath its space; a space with one tab also shows its panes directly beneath the space name. A space with exactly one tab and one pane uses a single numbered row with the space name, retaining pane focus, status, and shortcuts. Tabs with multiple panes in multi-tab spaces retain an unnumbered, selectable heading with numbered panes beneath it. Pane labels prefer the explicit pane name, then the pane title, and otherwise use `pane N`; agent types are not used as fallback names. Clicking or selecting a workspace, tab, or pane focuses it. Press `prefix+w` to focus the tree from the keyboard, then use `j`/`k`, `h`/`l`, Enter, and `/` for movement, collapse/expand, activation, and inline filtering. In Navigate mode, the selected row uses the active theme accent; outside Navigate mode, the currently focused pane keeps that accent background. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, mouse behavior, pane borders, and other presentation settings. `ui.agent_panel_sort` still controls Agent cycling and the compact collapsed rail; it does not reorder the hierarchical tree.
...
Set `hide_tab_bar = true` under `[ui]` to hide the desktop tab row regardless of tab count or position. Tabs remain accessible through the sidebar and configured keybindings. This takes precedence over `hide_tab_bar_when_single_tab`. Both options default to `false`.
...
### Sidebar tree node layouts

The existing Space and Agent layout settings style workspace nodes and detected-Agent pane nodes in the tree. Every tree node occupies one terminal line. Herdr resolves each inner array in `rows`, then flattens its nonempty values onto that node's line in order. These are the complete default layouts:
...
Tokens render in their configured order. Herdr normally separates adjacent values with ` · ` and uses a single space after `state_icon`. Missing values and their separators disappear; inner arrays with no values contribute nothing to the node line. Each layout may contain at most 16 inner arrays, with at most 16 tokens in each array. Existing custom rows remain unchanged, so add `machine` explicitly if you want machine identity in a custom multi-machine layout.

For a name-only tree without repeated parent names or agent types, keep only the
item name and optional status icon in each configured row:
...
The tree supplies the Machine → Space → Tab → Pane hierarchy. Its `pane` token
uses the pane row's resolved name, so an explicit pane name takes precedence over
an agent-reported title.
...
Tree nodes are deliberately packed one per terminal row, so `row_gap` is retained as a compatibility setting but does not add spacing in the expanded tree.

Override the complete Agent pane-node layout for a known agent under `rows_by_agent`:
...
An override replaces `rows`; it does not extend it. Its inner arrays are flattened onto the pane node in the same way. Override keys are case-sensitive canonical agent IDs such as `claude`, `codex`, and `pi`. Detection aliases such as `claude-code` are not accepted. Agents without an override, including custom reported agents, use `rows`.
...
Sidebar node settings affect only the expanded desktop tree. Shell panes use their resolved pane title or label because they have no Agent token context. Collapsed and mobile views keep their compact layouts.
