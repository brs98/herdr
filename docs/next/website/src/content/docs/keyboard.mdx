...
| Sidebar tree navigation | `prefix+w` |
...
Press `prefix+w` to focus the expanded desktop sidebar. Herdr selects the currently focused pane in the workspace → tab → pane tree; if the sidebar is collapsed, it opens first. With several connected machines, the tree groups sessions under their machine. Selecting a session on another online machine activates that machine. Cached offline sessions remain visible but cannot be activated.

- Move through visible rows with `up`/`down` or `k`/`j`.
- Use `left`/`h` to collapse an open workspace or tab, then move to its parent. Use `right`/`l` to expand it, then move to its first child. Space toggles the selected workspace or tab.
- Press Enter to focus the selected workspace, tab, shell pane, or Agent pane. Moving the tree cursor alone does not change terminal focus.
- Press `/` to filter the tree in place. Type or paste a query, use Backspace or `ctrl+u` to edit it, and keep moving with `up`/`down`. Esc returns to the tree while keeping the filter; press Esc again to leave Navigate mode.

`prefix+g` remains the larger session navigator, including Agent-state filters, when you want a fast picker instead of working in the sidebar.
